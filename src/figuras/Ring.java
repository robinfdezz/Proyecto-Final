...
public class Ring extends Figura {
    // Usamos puntoInicial como centro y puntoFinal como punto en el radio exterior
    // protected Point puntoInicial; // Heredado de Figura, representa el centro
    // protected Point puntoFinal;   // Heredado de Figura, representa un punto en el radio exterior
    ...
    public void actualizar(Point puntoActual) {
        this.puntoFinal = puntoActual; // Usamos puntoFinal como punto en el radio exterior
    }
    ...
    public void dibujar(Graphics g) {
        ...
        int radioExterior = (int) puntoInicial.distance(puntoFinal); // Calcular el radio exterior
        ...
        int xExt = puntoInicial.x - radioExterior;
        int yExt = puntoInicial.y - radioExterior;
        int diamExt = radioExterior * 2;

        int xInt = puntoInicial.x - radioInterior;
        int yInt = puntoInicial.y - radioInterior;
    ...
    public FiguraData getFiguraData() {
        ...
        data.setPuntoInicial(this.puntoInicial); // Centro
        data.setPuntoFinal(this.puntoFinal);   // Punto en el radio exterior
        data.setColorDePrimerPlano(this.colorDePrimerPlano);
        data.setColorDeRelleno(this.colorDeRelleno);
        data.setEstaRelleno(this.relleno);
        // No tiene sentido para Ring setear puntosTrazo o tamanoBorrador
        data.setCentro(this.puntoInicial); // Guardar el centro explícitamente también por claridad
        return data;
    }
    ...
    public boolean contains(Point p) {
        ...
        int radioExterior = (int) puntoInicial.distance(puntoFinal);
        int radioInterior = (int) (radioExterior * 0.6); // Debe coincidir con el cálculo en dibujar

        double distanciaAlCentro = puntoInicial.distance(p);
    ...
    public Rectangle getBounds() {
        if (puntoInicial != null && puntoFinal != null) {
            int radioExterior = (int) puntoInicial.distance(puntoFinal);
            int x = puntoInicial.x - radioExterior;
            int y = puntoInicial.y - radioExterior;
